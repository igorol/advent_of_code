...
class KeepAway:
    ...
    def initialize_monkeys(self):
        ...
        return [self._parse_note(monkey_id, note) for monkey_id, note in enumerate(notes)]
    ...
    def play_monkey(self, monkey_id):
        ...
        for old_worry in worry_levels:
            if self.worry_divisor == 1:
                new_worry = eval(f"{old_worry} {self.monkeys[monkey_id].operation}") % self.common_divisor
            else:
                new_worry = int(eval(f"{old_worry} {self.monkeys[monkey_id].operation}") / self.worry_divisor)
        ...
        _ = [self.monkeys[monkey_id].worry_levels.remove(level) for level in worry_levels]
