...
class Cave:
    ...
    def __init__(self, cave_dims=(1000, 300), sand_source=(500, 0), fname="test_input"):
        ...
        self.fname = fname
        self.cave_array = self.initialize_cave()
        self.counter = 0
        self.abyss_depth = np.where(self.cave_array == self.ROCK)[1].max()
    ...
    def update_cave(self, new, old, stop=False):
        self.cave_array[new] = self.FALLING_SAND
        self.cave_array[old] = self.STILL_SAND if stop else self.FREE
        return new, old

    def render_cave(self, fname=None):
        data = self.cave_array[325:675, :170].T
    ...
    def sand_falls(self, render=False):
        ...
        self.cave_array[pos] = self.FALLING_SAND

        if self.cave_array[pos[0], pos[1] + 1] in self.UNAVAILABLE:
        ...
        while True:
            ...
            if self.cave_array[pos[0], pos[1] + 1] not in self.UNAVAILABLE:
                pos, previous_pos = self.update_cave((pos[0], pos[1] + 1), pos)
            elif self.cave_array[pos[0] - 1, pos[1] + 1] not in self.UNAVAILABLE:
    ...
    def part1(self):
        while 1:
            if self.sand_falls():
                break
        return np.count_nonzero(self.cave_array == self.STILL_SAND)

    def part2(self):
        self.abyss_depth += 2
        self.cave_array[:, self.abyss_depth + 1] = self.ROCK
        while 1:
            if self.sand_falls(render=False):
                break
        return np.count_nonzero(self.cave_array == self.STILL_SAND) + 2
