...
def parse_line(line):    
    input_parser = re.compile(r"(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)")
...
def part1(data, grid):
    for line in data:
        ...
        if instruction == "turn on":
            ...
            grid[y1 : y2 + 1, x1 : x2 + 1] = np.logical_not(grid[y1 : y2 + 1, x1 : x2 + 1])
...
def part2(data, grid):
    for line in data:
        ...
        if instruction == "turn on":
            ...
            grid[y1 : y2 + 1, x1 : x2 + 1] -= 1
            grid[y1 : y2 + 1, x1 : x2 + 1] = np.maximum(0, grid[y1 : y2 + 1, x1 : x2 + 1])
        elif instruction == "toggle":
